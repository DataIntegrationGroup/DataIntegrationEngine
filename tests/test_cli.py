...
def _timeseries_tester(source, func, args=None, combined_flag=True, timeseries_flag=True):
    runner = CliRunner()

    nosources = [f for f in ('--no-amp',
                             '--no-nwis',
                             '--no-st2',
                             '--no-bor',
                             '--no-dwb',
...
def test_unify_waterlevels_nwis():
    _timeseries_tester('nwis', waterlevels,
